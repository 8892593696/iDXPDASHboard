...
import com.pilog.mdm.cloud.DAO.DashBoardsDAO;
import com.pilog.mdm.cloud.DAO.IDXPGridProcessDAO;
import com.pilog.mdm.cloud.Utils.DashBoardUtills;
import com.pilog.mdm.cloud.vision.utilities.NumberGeneration;
...
public class DashBoardsService {
    ...
    @Autowired
    public IDXPGridProcessDAO processDao;
    @Autowired
    private NumberGeneration generation;
    @Autowired
    public DashBoardUtills dashBoardUtills;

    private PiLogCloudUtills cloudUtills = new PiLogCloudUtills();
...
//    }
    ...
    public String getVisualizationLayout(HttpServletRequest request) {
        ...
        try {
            ...
            result = "<div class='dxpDataAnalyticswrapper'>"
                    ...
                    + "<div class='col-lg-4  col-md-4 visualIconDivImg'><img onclick=\"getChartDiv('ganttchart.png', 'ganttChart')\" src='images/ganttChart.png' class='visualDarkMode' title='GanttChart'></div>"
                    + "<div class='col-lg-4  col-md-4 visualIconDivImg'><img onclick=\"getChartDiv('stackedBarChart.svg', 'stackedBarChart')\" src='images/stackedBarChart.svg' class='visualDarkMode' title='A stacked bar chart is a type of bar chart where each bar is divided into multiple segments, with each segment representing a different category or sub-group. The length of each segment shows the proportion of each category within the total, making it easy to compare both the overall size and the individual components across different categories or time periods'></div>"
                    + "</div>" + "</div>" + "</div>"
    ...
    public JSONArray getDataMigrationConnectionsTreeMenu(HttpServletRequest request, String parentMenuId) {
        ...
        try {
            String dmaFlag = request.getParameter("dmaFlag");
            if (parentMenuId != null && !"Y".equalsIgnoreCase(dmaFlag)) {
                ...
                menuObj5.put("value", "javascript:getTreeDataBase('FILE','CSV')");
                ...
                menuObj8.put("value", "javascript:getTreeDataBase('FILE','JSON')");
                ...
                menuObj9.put("value", "javascript:getTreeDataBase('FILE','XML')");
                ...
                menuObj10.put("value", "javascript:getTreeDataBase('FILE','TEXT')");
...
//				String filePath = "C:/Files/TreeDMImport" + File.separator + userName;
                ...
                String dmaReportFlag = request.getParameter("dmaReportFlag");
...
//                Workbook outputWb = (XSSFWorkbook) WorkbookFactory.create(new File(fileName));
                ...
                for (Object sheet : sheetsArray) {
                    ...
                    if (dmaReportFlag != null && !"".equalsIgnoreCase(dmaReportFlag) && !"null".equalsIgnoreCase(dmaReportFlag) && "Y".equalsIgnoreCase(dmaReportFlag)) {
                        sheetHeaders.add("BATCH_ID");
                        sheetHeaders.add("RESPONSE_TITLE");
                    }
                    ...
                    for (int i = 0; i < sheetData.size(); i++) {
                        ...
                        if (dmaReportFlag != null && !"".equalsIgnoreCase(dmaReportFlag) && !"null".equalsIgnoreCase(dmaReportFlag) && "Y".equalsIgnoreCase(dmaReportFlag)) {
                            rowData.put("BATCH_ID", batchNumber);
                        }
                        if (rowData != null) {
...
//                    dashBoardsDAO.saveUserFiles(request, originalFileName, mainFileName, filePath, selectedFiletype);
                ...
                importResult = getFileObjectMetaData(request, response, fileName, gridId, selectedFiletype,
                        mainFileName);
                importResult.put("datafilds", dashBoardsDAO.showtableData(request));
...
//                        char columnSeparator = ',';
                        ...
                        CSVParser parser1 = new CSVParserBuilder().withSeparator(columnSeparator).build();
                        CSVReader reader = new CSVReaderBuilder(
                                new InputStreamReader(new FileInputStream(filePath), StandardCharsets.UTF_8))
                                .withCSVParser(parser1)
                                .build();
...
//            char colSepartor = ',';
           ...
           CSVParser parser1 = new CSVParserBuilder().withSeparator(colSepartor).build();
                        CSVReader reader = new CSVReaderBuilder(
                                new InputStreamReader(new FileInputStream(filepath), StandardCharsets.UTF_8))
                                .withCSVParser(parser1)
                                .build();
...
//				+ "</div>" + "</li>";
    ...
    public String getFontObject(String layoutType, String fontKey) {
        ...
        String fontObject = "<div class=\"main-container inner-container\">" + "<div class=\"filter-container\">"
                ...
                + "</div>"
                // + getToggleButton()
                + "</div>" + "<ul id=\"" + layoutType + "FONT\" data-opt-name=\"" + fontKey
                ...
                + "<label>Font Family</label>" + "<input type=\"text\" id=\"" + layoutType
                + "FONTFAMILY\" data-opt-name=\"family\" data-man=\"O\"/>" + "</li>"
    ...
    public String getSliceLabelsECharts(String chartType) {
        String sliceLabels = "<li id=\"label-filter\" data-column-name=\"" + chartType
                ...
                + "</li>" + getFontListForECharts(chartType) + "</ul>" + "</li>";
        return sliceLabels;
    ...
    public String getFontListForECharts(String chartType) {
        String fontList = "<li class=\"sub-filterItems active-filter\" data-column-name=\"" + chartType
                ...
                + "<option value=\"Courier New\">Courier New</option>" + "</select>" + "</li>";
        return fontList;
    }
    ...
    public String getFontObjectEcharts(String chartType) {
        String fontObject = "<div class=\"main-container inner-container\">" + "<div class=\"filter-container\">"
                ...
                + "<option value=\"sans-serif\">Sans Serif</option>" + "<option value=\"serif\">Serif</option>"
                + "<option value=\"monospace\">Monospace</option>" + "<option value=\"Arial\">Arial</option>"
                + "<option value=\"Courier New\">Courier New</option>" + "</select>" + "</li>" + "</ul>";
        return fontObject;
    }
...
//        String tablename = request.getParameter("table");
    ...
    public List getSerachResults(List gridColArray, String tableName, String gridId, HttpServletRequest request) {
        ...
        try {
            ...
            dataList = processDao.getSerachResults(gridColArray, tableName, gridId, request, paramArray);
...
//        String tablename = request.getParameter("table");
    ...
    public void downloadALLChartsInPDF(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String chartImageperrow = request.getParameter("chartImageperrow");
            int imgperrow = 2;
            try {
                if (chartImageperrow != null && !"".equalsIgnoreCase(chartImageperrow) && !"null".equalsIgnoreCase(chartImageperrow)) {
                    imgperrow = Integer.parseInt(chartImageperrow);
                }
            } catch (Exception e) {
                imgperrow = 2;
            }
...
//                }
    ...
    public JSONObject getChartHomePageDiv(HttpServletRequest request) {
        ...
        try {
            String divStr = "<div id=\"mainintelliSenseSelectBoxId\" class='intelliSenseSelectBoxClass'>"
                    + "<div id='mainintelliSenseInnerSelectBoxId'class='intelliSenseSelectBoxClass'></div>"
                    //                    + "<div id=\"chartSelectionsDropDownId\" class=\"chartSelectionsDropDown\" style=\"display:none\">"
                    + "<div id=\"chartSelectionsDropDownId\" class=\"chartSelectionsDropDown\" >"
                    ...
                    + "</div>" + "<section class=\"visualizationDashboardView\" style=\"display:none\">"
                    ...
                    + "<input type=\"hidden\" value=\"\" id=\"chartImageperrow\" name=\"chartImageperrow\"/>\n"
    ...
    public JSONObject getcolorpalleteform(HttpServletRequest request) {
        ...
        try {
            ...
            result += "<div class='themeBtnsContainer'>" //+ " <div class='colorblockTitle'><h5>Palette</h5></div>"
                    ...
                    //+ " <span class='themeBtns' data-color='#2bc4ff' style='background-color: #2bc4ff;'></span>"ḍ
                    + "</div>" + "  </div>"
                    + "<div class='themeBtnsGroup' onclick=saveGrpahColors(event,'" + chartid
                    + "') tittle=''Click to Save Color onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\">"
                    + "<div>"
                    + "<span class='themeBtns' data-color='#4b6043' style='background-color:#4b6043;'></span>"
                    ...
                    + "') tittle=''Click to Save Color onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\">"
                    + "<div>"
                    + "<span class='themeBtns' data-color='#19362d' style='background-color:#19362d;'></span>"
                    ...
                    + "</div>" + "  </div>"
                    + "<div class='themeBtnsGroup' onclick=saveGrpahColors(event,'" + chartid
                    + "') tittle=''Click to Save Color onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\"><div>"
                    ...
                    + "</div>" + "  </div>"
                    + "<div class='themeBtnsGroup' onclick=saveGrpahColors(event,'" + chartid + "') title='Click to Save Color' "
                    + "onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\">"
                    + "<div>"
                    ...
                    + "</div>" + "</div>"
                    + "<div class='themeBtnsGroup' onclick=saveGrpahColors(event,'" + chartid + "') title='Click to Save Color' onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\">"
                    + "<div>"
                    + " <span class='themeBtns' data-color='#0474cc' style='background-color: #0474cc;'></span>"
                    + "<span class='themeBtns' data-color='#1c7ab5' style='background-color: #1c7ab5;'></span>"
                    + " <span class='themeBtns' data-color='#149ea8' style='background-color: #149ea8;'></span>"
                    + "<span class='themeBtns' data-color='#CFE8F9' style='background-color: #CFE8F9;'></span>"
                    + " <span class='themeBtns' data-color='#7caecb' style='background-color: #7caecb;'></span>"
                    + "</div><div>"
                    + " <span class='themeBtns' data-color='#FFECB3' style='background-color: #FFECB3;'></span>"
                    + " <span class='themeBtns' data-color='#FFCDD2' style='background-color: #FFCDD2;'></span>"
                    + " <span class='themeBtns' data-color='#E0E0E0' style='background-color: #E0E0E0;'></span>"
                    + " <span class='themeBtns' data-color='#424242' style='background-color: #424242;'></span>"
                    + " <span class='themeBtns' data-color='#9E9E9E' style='background-color: #9E9E9E;'></span>"
                    + "</div>" + "</div>"
                    + "<div class='themeBtnsGroup' onclick=saveGrpahColors(event,'" + chartid + "') title='Click to Save Color' onmouseover=\"updatecolorOnGraph(event,'" + chartid + "')\">"
                    + "<div>"
                    + "<span class='themeBtns' data-color='#64B5F6' style='background-color: #64B5F6;'></span>"
                    + "<span class='themeBtns' data-color='#2196F3' style='background-color: #2196F3;'></span>"
                    + " <span class='themeBtns' data-color='#E3F2FD' style='background-color: #E3F2FD;'></span>"
                    + " <span class='themeBtns' data-color='#F5F5F5' style='background-color: #F5F5F5;'></span>"
                    + " <span class='themeBtns' data-color='#90CAF9' style='background-color: #90CAF9;'></span>"
                    + "</div><div>"
                    + " <span class='themeBtns' data-color='#212121' style='background-color: #212121;'></span>"
                    + " <span class='themeBtns' data-color='#757575' style='background-color: #757575;'></span>"
                    + " <span class='themeBtns' data-color='#66BB6A' style='background-color: #66BB6A;'></span>"
                    + " <span class='themeBtns' data-color='#FFB300' style='background-color: #FFB300;'></span>"
                    + " <span class='themeBtns' data-color='#EF535' style='background-color: #EF535;'></span>"
                    + "</div>" + "</div>"
                    + " </div>" + " </div>" + "</div>";
...
//              buttonDiv="<span><img src='images/split.png' width='16px' title='split data' onclick= splitData('"+tableName+"')>";
                buttonDiv = "<span><img src='images/Delete-Icon-03-01.png' title='Delete Column' onclick=deleteColumn("
                ...
                buttonDiv += "<img src='images/Dimention-Transpsose-Icon-02-01.png' title='Dimension transpose' onclick= DimensionTranspose(event,'"
                        + tableName + "')>";
                buttonDiv += "<img src='images/Change Datatype-Icon-03-01.png' title='Table View' onclick= ChooseOptions(event,'"
                        + tableName + "')>";
                buttonDiv += "<img src='images/Export-Icon-03-01.png' title='Export'id ='ExportgridId' onclick= generateexcel>";
                buttonDiv += "<img src='images/Chart Auto-Suggetion-Icon-03-01.png' title='Chart Suggestions' onclick= getModalFileColumns(event,'"
                        + tableName + "')>";
                buttonDiv += "<img src='images/Pivot Descriptor-Icon-03-01.png' title='Pivot table' onclick= getCrossTabData('" + tableName
                        + "')>";
                buttonDiv += "<img src='images/Pivot table-Icon-03-01.png' title='Pivot table' onclick= getPivotGridData('"
                        + tableName + "')>";
                buttonDiv += "<img src='images/Pivot table-Icon-03-01.png' title='Data correlation' onclick= getDataCorrelation('"
                        + filePath + "')>";
                buttonDiv += "</span>";
...
//                                            outputCell.setCellType(CellType._NONE);
    ...
    public JSONObject getsearchChartData(HttpServletRequest request) {
        return dashBoardsDAO.getsearchChartData(request);
    }
...
//MCA Analysis
    public void chartInPDF(String imageStr, HttpServletResponse response, ModelMap model, HttpServletRequest request, String filename) {
        OutputStream os = null;
        com.itextpdf.text.Document document = null;
...
//            System.out.println("rect::" + rect);
            ...
            HeaderAndFooterPdfPageEventHelper headerAndFooter = new HeaderAndFooterPdfPageEventHelper(request, writer, document);
...
//            document.add(new Paragraph(Chunk.NEXTPAGE));
            if (analysisType != null && !"".equalsIgnoreCase(analysisType) && !"null".equalsIgnoreCase(analysisType)
                ...
                Paragraph firstPagetext3 = new Paragraph("Material Criticality Assessment\n(Inventory Management)", new Font(customBaseFont, 20, Font.BOLD, BaseColor.BLACK));
...
//                    abcPdfData(imageStr, request, document, analysisType);
    ...
    public void abcPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
...
//            Font f = new Font(customBaseFont, 16.0f, Font.BOLD, BaseColor.WHITE);
            ...
            Chunk firstMainHeaddingChunk = new Chunk("1.Introduction :", new Font(customBaseFont, 16.0f, Font.BOLD, BaseColor.WHITE));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            //onkar color
            Chunk firstSubHeaddingChunk = new Chunk("Material Criticality Analysis", new Font(customBaseFont, 14.0f, Font.BOLD, BaseColor.WHITE));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            firstMainHeaddingData.add(new Paragraph("   Material Criticality and Inventory Analysis is performed in order to determine the criticality, consumption value, consumption rate, stock value, "
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingData = new Paragraph("The following are the various methods to control the inventory process:", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            Paragraph bulletSubHeadding1 = new Paragraph("        It classifies the materials based on their consumption during a particular   time period (usually one year)", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletSubHeadding2 = new Paragraph("        It is based on the rate of issue or rate of usage of spare parts. This classification system categorizes the items based on how frequently"
                    + " the parts are issued and how frequently they are used", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletSubHeadding3 = new Paragraph("        It classifies the materials based on stock value accumulation. It is calculated by dividing an item's current stock value by the "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletSubHeadding4 = new Paragraph("        It classifies the materials according to their criticality and impact to the production process or other services i.e. how and to what extent the material "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletSubHeadding5 = new Paragraph("        It classifies the materials based on their unit prices. The main objective of this analysis is to minimize the inventory cost such as "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletSubHeadding6 = new Paragraph("        It classifies inventory based on how freely available an item or scarce an item is, or the length of its lead time.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            secondMainHeaddingData.add(new Paragraph("    Always Better Control (ABC) approach is a popular and effective method used to classify inventory items into specific categories that can "
            ...
            rowsDataTablevalue = new Paragraph("Decisions to be consider", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("In This Concept", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("How to determine relative ranking of each item?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Create an ABC class", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("How to arrange the items in the decreasing order based on the defined criteria?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Create an ABC classification set", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("How to associate ABC classes with ABC classification sets?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Create ABC assignment groups", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("How to associate items to ABC classes within an ABC assignment group?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Perform ABC assignments", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Can we update ABC assignments?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("                    Yes", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("Can we purge an ABC assignment group?", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
            rowsDataTablevalue = new Paragraph("                    Yes", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            rowsDataTable.addCell(rowsDataTablevalue);
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            thirdSubHeaddingData.add(new Paragraph("   In ABC Analysis, the items are classified into A, B, C category in which \n"
                    ...
                    + "C - items are the items with the lowest consumption value the lower 5% of the annual consumption value typically accounts for 50% of total inventory items.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            fourthSubHeadingData.add(new Paragraph("    1) Prepare the list of items and estimate their annual consumption (units).\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            fourthSubHeadingData.add(new Paragraph("    2) Determine unit price (or cost) of each item.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            fourthSubHeadingData.add(new Paragraph("    3) Multiply each annual consumption by its unit price (or cost) to obtain its annual consumption in rupees (annual usage).\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            fourthSubHeadingData.add(new Paragraph("    4) Arrange items in the descending order of their annual usage starting with the highest annual usage down to the smallest usage.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            fourthSubHeadingData.add(new Paragraph("    5) Calculate cumulative annual usages and express the same as cumulative usage percentages. Also express the number of items into cumulative item percentages.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            fourthSubHeadingData.add(new Paragraph("    6) Graph cumulative usage percentages against cumulative item percentages and segregate the items into A, B and C categories.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(fourthSubHeadingData);
            document.add(new Paragraph(Chunk.NEWLINE));

            //bg color
            Chunk fifthMainHeaddingChunk = new Chunk("" + analysisType + " Analysis - Based on consumption value per year\n", new Font(customBaseFont, 14, Font.BOLD, BaseColor.WHITE));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            fifstMainHeaddingData.add(new Paragraph("  " + analysisType + "  analysis classifies the materials based on their consumption during a particular time period (usually one year)\n"
                    ...
                    + "Note: An A class item need not necessarily be a fast-moving item. Alternatively, C class may or may not be a fast-moving item. ABC analysis is purely based on the dollar value of consumptionn", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            cellvalue = new Paragraph("It represents the most valuable products. These are the products that contribute heavily to your overall profit without "
                    ...
                    + "It requires tight control", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Cumulative percent: 0-75%\n"
                    + "It represents the most valuable products", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("    B", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("It represents the middle products. These are the products that contribute to the bottom line but aren’t significant enough to "
                    + "receive a lot of attention. It requires moderate control", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Cumulative percent: >75-90%\n"
                    + "It represents the middle products", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("    C", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("It represents the hundreds of tiny transactions that are essential for profit but don’t individually contribute much value "
                    + "to the company. This is the category where most of the products will live. It is also the category where we must try to automate sales as much as possible to drive down overhead costs. It requires least control", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Cumulative percent: >90-100%\n"
                    + "It represents the hundreds of tiny transactions that are essential", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            ...
            sixthMainHeaddingData.add(new Paragraph("    2) Collect all the data about the inventory and calculate the consumption or sale value\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            sixthMainHeaddingData.add(new Paragraph("    3) Arrange all the consumption values in descending order of values\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            sixthMainHeaddingData.add(new Paragraph("    4) Create next column and start adding the cumulative total of consumption value\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            sixthMainHeaddingData.add(new Paragraph("    5) Consumption is considered for 5 years’ data and results are calculated as average for one year\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            cellvalue = new Paragraph("           A", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
            cellvalue = new Paragraph("                            Cumulative percent: 0-75%", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
            cellvalue = new Paragraph("           B", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
            cellvalue = new Paragraph("                            Cumulative percent: >75-90%", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
            cellvalue = new Paragraph("           C", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
            cellvalue = new Paragraph("                            Cumulative percent: >90-100%", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            countTable.addCell(cellvalue);
//            countTable.setSpacingBefore(30f);
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            //bg color
            Chunk eightMainHeaddingChunk = new Chunk("Sample Graphical Representation Of " + analysisType + " Analysis:\n", new Font(customBaseFont, 14, Font.BOLD, BaseColor.WHITE));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue = new Paragraph("The adaptation of Pareto's Law of the vital few and trivial many follows pattern:", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue1 = new Paragraph("Category ‘A’ consists of items of high value but small in numbers.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue1);
            Paragraph ninethMainHeaddingValue2 = new Paragraph("Category ‘B’ kept in between A and C items. ‘B’ items have similar controls to ‘A’ items but are less frequent.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue2);
            Paragraph ninethMainHeaddingValue3 = new Paragraph("Category ‘C’ items have the simplest controls. They are only important if there is a shortage of one of them.  Thus ‘C’ category  "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            bulletMainHeaddings9.add(new Phrase("  A category inventory items where annual consumption value is the highest. Applying the Pareto principle (also referred to as the 80/20 rule where 80 "
                    ...
                    + "analysis and control of this class is relatively intense since there is the greatest potential to reduce costs or losses.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddings9);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddings10 = new Paragraph();
            bulletMainHeaddings10.add(bullet);
            bulletMainHeaddings10.add(new Phrase(" B category inventory items are inter class items. Their consumption values are lower than A items but higher than C items. A key point of having this inter-class group is to "
                    ...
                    + "the value at risk of loss or the cost of analysis and the potential value returned by reducing class costs.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddings10);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddings11 = new Paragraph();
            bulletMainHeaddings11.add(bullet);
            bulletMainHeaddings11.add(new Phrase(" C category inventory items have the lowest consumption value. This class has a relatively high proportion of the total number of lines but with relatively low consumption values."
                    ...
                    + "relatively low returns.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingData = new Paragraph("ABC analysis of inventory helps you keep working capital costs low because it identifies which items you should reorder more frequently "
            ...
            thirdMainHeaddingData1.add(new Phrase(" Better control over high-value inventory improves availability and reduces losses and costs.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(thirdMainHeaddingData1);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingData2 = new Paragraph();
            thirdMainHeaddingData2.add(bullet);
            thirdMainHeaddingData2.add(new Phrase(" More efficient use of stock management resources. During stock count more resources are dedicated to A class than B or C class holdings or "
            ...
            thirdMainHeaddingData3.add(new Phrase(" Relatively low value of B or C class holdings can allow a business to hold bigger buffer stocks to reduce stock outs.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            thirdMainHeaddingData5.add(new Phrase(" Fewer stock outs and improved production efficiency resulting in more reliable cycle time and improved customer satisfaction.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            Paragraph fourthSubMainHeaddingData1 = new Paragraph("ABC analysis of inventory helps you effectively forecast demand by splitting companies inventory into categories "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fourthSubMainHeaddingData2 = new Paragraph("After analyzing your inventory and splitting inventory into A, B or C categories  the professionals need to know which "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fourthSubMainHeaddingData3 = new Paragraph("Since the items are category A are best-sellers  and company may be able to raise the prices of those products for increased revenue.\n"
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fourthSubMainHeaddingData = new Paragraph("ABC analysis is an important technique in materials management. It includes of supply chain functions including sourcing, procuring, "
                    ...
                    + "counted less frequently and more leniently controlled.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(fourthSubMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph tenthMainHeadding = new Paragraph("ABC classification is important in inventory management for several reasons. It allows supply chain managers to:\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            Paragraph tenthMainHeaddingData = new Paragraph();
            tenthMainHeaddingData.add(new Paragraph("    1) Identify the inventory items that pose the biggest business risks due to theft or damage, and pose the largest opportunities from to sales\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            tenthMainHeaddingData.add(new Paragraph("    2) Help warehouse managers and other supply chain professionals to properly prioritize their time\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            tenthMainHeaddingData.add(new Paragraph("    3) Empower warehouse managers to achieve close to 100% inventory accuracy\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph SubMainHeadding2 = new Paragraph("Some of the supply chain professionals categorize items based on how frequently they movement. In some instance frequently ordered items would sit in Category A,"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph SubMainHeadding3 = new Paragraph("Finally, the justification for this method is that fast moving items are more likely to experience stockouts. This means they are more susceptible to loss, "
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph SubMainHeadding4 = new Paragraph("Some of the supply chain managers prefer to categorize items based on their value. The most expensive items fall into Category A. items with an average price fall"
                    + " into Category B and the cheapest items fall into Category C.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(SubMainHeadding4);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph SubMainHeadding5 = new Paragraph("Final justification for this method is that these items represent the highest individual sales for a company and the biggest potential loss. "
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            fourthMainHeaddingdata.add(new Phrase(" The method we will use to conduct an ABC analysis takes both factors into consideration, the demand and the cost of the inventory material.\n"
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingData1.add(new Phrase(" The concept of ABC classification is a simple process of classifying material in to large, complex inventories. While classifying your inventory the first time a lot "
                    ...
                    + "Once started conducting an ABC analysis of inventory the following steps has to follow.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue1.add(new Phrase(" Initially create two item lists, One list is for items that are purchased and the other is for items that are manufactured. In this way we have a "
                    ...
                    + "Have to focus on inventory value. We can focus mainly on all purchased items even if they aren’t directly related to production.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(ninethsubMainHeaddingDataValue1);
            document.add(new Paragraph(Chunk.NEWLINE));

            //bg color
            Chunk ninethsubMainHeaddingDataStep2Chunk = new Chunk("Step 2. Collect the Standard Cost and Annual Usage Data\n", new Font(customBaseFont, 14, Font.BOLD, BaseColor.WHITE));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue2.add(new Phrase(" In a spreadsheet( excel ), we can segregate the material in three columns. The first column should be a list of the part numbers for"
                    ...
                    + "each item. Here demand should also be in consumption quantity..", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue3.add(new Phrase(" Once collected the consumption and cost for the materials calculation of annual spend. Simply multiply the standard unit cost of each part by its "
                    ...
                    + "calculate the total cumulative annual spend for materials.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue4.add(new Phrase(" The Cumulative annual spend is simply a running total of each item’s annual spend that accumulates to equal the total annual spend of all items."
                    + " The cumulative running total should be equal to the sum of the annual spend of itself plus the annual spend of all items..", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue5.add(new Phrase(" To identify which parts should be classified as class A items you must first find the point that can spend annual of 75% of the total cost.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(new Paragraph(Chunk.NEWLINE));
            ninethsubMainHeaddingDataValue5.add(new Phrase(" Target Total Annual Spend = Total annual spend for all purchased items x 75%", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue6.add(new Phrase(" Class B items will represent the next 15% of your total annual spend. To find the separating class B items from class C items determine the value "
                    ...
                    + "Make manual adjustments so that your cut-off point is meaningful as you did with the A items. Designate the appropriate non-A items below your identified as B items.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            ninethsubMainHeaddingDataValue7.add(new Phrase(" All remaining items that do not meet the requirements to be classified as Class A or Class B are simply categorize with  “C” in the ABC "
                    ...
                    + "You can choose all these and plot a graph for the same by using cumulative percentages…", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            cellvalue=new Paragraph(,new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            ...
            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("ACount") != null ? request.getParameter("ACount") : "0") + Integer.parseInt(request.getParameter("BCount") != null ? request.getParameter("BCount") : "0") + Integer.parseInt(request.getParameter("CCount") != null ? request.getParameter("CCount") : "0")));
            reportTable.addCell("");
            reportTable.addCell("");
            ...
            Paragraph endingLine = new Paragraph("              Properly managing inventory can make or break a business, and inventory planning is essential to this process. Having insight into your stock at any given"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph endingquote2 = new Paragraph("\"As renowned partner of choice, we are here to assist in journey to excel\"", new Font(customBaseFont, 14, Font.BOLD, BaseColor.BLACK));
            document.add(endingquote2);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph endingquote3 = new Paragraph("Learn more about how you can explore the Digital Analytical Tools in our \"Data Quality Hub\" to transform your Business", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(endingquote3);
            //link started
            document.add(new Paragraph(Chunk.NEXTPAGE));
            Paragraph recommendedAnalysis = new Paragraph("Recommended Other PiLog Products :", new Font(customBaseFont, 16, Font.BOLD, BaseColor.BLACK));
            ...
            anchor.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=VED_ANALYSIS_TAB&highLevelMenu=VED_ANALYSIS");
            document.add(anchor);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor1 = new Anchor("SDE Analysis (Based on Lead Time & Availability of Items)\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor1.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=SDE_ANALYSIS_TAB&highLevelMenu=SDE_ANALYSIS");
            document.add(anchor1);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor2 = new Anchor("HML Analysis (Based on Unit Price of the Material)\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor2.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=HML_ANALYSIS_TAB&highLevelMenu=HML_ANALYSIS");
            document.add(anchor2);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor3 = new Anchor("FMSN Analysis (Based on Consumption Rate)\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor3.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=FMSN_ANALYSIS_TAB&highLevelMenu=FMSN_ANALYSIS");
            document.add(anchor3);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor10 = new Anchor("XYZ Analysis (Based on Stock Value Accumulation)\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor10.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=XYZ_ANALYSIS_TAB&highLevelMenu=XYZ_ANALYSIS");
            document.add(anchor10);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor4 = new Anchor("Overview of Vendor Data Profiling.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor4.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=VENDOR_DATA_PROFILING_TABS&highLevelMenu=VENDOR_DATA_PROFILING");
            document.add(anchor4);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor5 = new Anchor("Overview of Master Data Profiling.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor5.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=MATERIAL_DATA_PROFILING_TABS&highLevelMenu=MATERIAL_DATA_PROFILING");
            document.add(anchor5);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor6 = new Anchor("Overview of Customer Data Profiling.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor6.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=CUSTOMER_DATA_PROFILING_TABS&highLevelMenu=CUSTOMER_DATA_PROFILING");
            document.add(anchor6);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor7 = new Anchor("ISIC Classification Allocation\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor7.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=ISIC_ALLOCATION_TABS&highLevelMenu=ISIC_ALLOCATION");
            document.add(anchor7);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Anchor anchor8 = new Anchor("HSN Code Allocation\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLUE));
            anchor8.setReference("https://imdrm.pilog.in/PiLogCloud/cloudTabs?tabId=HSN_ALLOCATION_TABS&highLevelMenu=HSN_ALLOCATION");
            document.add(anchor8);
//            document.add(new Paragraph(Chunk.NEWLINE));

            //SDE Analysis (SDE analysis classifies inventory based on how freely available an item or scarce an item is, or the length of its lead time.) 
            //HML Analysis  (HML analysis classifies the materials based on their unit prices.) 
            document.close();
    ...
    public void xyzPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            BaseColor backgroundBaseColor = new BaseColor(64, 135, 94);//green
//            BaseColor backgroundBaseColor=new BaseColor(0, 113, 197);//blue
            PdfPCell cell = new PdfPCell();
            ...
            document.add(firstMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingData = new Paragraph("The following are the various methods to control the inventory process:\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            ...
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("XYZ analysis classifies the materials based on stock value accumulation. It is calculated by dividing an item's current stock value by the total "
            ...
            bulletHeaddings1.add(new Phrase(" X – very little variance: X materials are characterized by a constant, non-changing usage over time. The requirements fluctuate only slightly around a constant"
            ...
            bulletHeaddings2.add(new Phrase(" Y – some variance: The usage of Y materials is neither constant nor sporadic. With Y materials, we can often observe trends, for example, that the usage increases "
            ...
            bulletHeaddings3.add(new Phrase(" Z – the most variation: Z materials are not used regularly. The usage can strongly fluctuate or occur sporadically. In these cases, we can often observe periods "
                    + "with no consumption at all.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings3);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings4 = new Paragraph();
            bulletHeaddings4.add(new Phrase(" To arrange products according to their consumption (turnover is constant, fluctuating, irregular) to derive an optimal inventory strategy. The XYZ analysis gives,"
            ...
            bulletHeaddings5.add(new Phrase("The XYZ analysis gives, you an immediate view of which items are expensive to hold. Through this analysis, you can reduce your money locked up by keeping as little "
            ...
            dataTable.addCell("Y");
            dataTable.addCell("Items which are of lower criticality requiring standard controls and periodic reviews of usage");
            dataTable.addCell("Cumulative percent: >75-90%\n"
                    + "It represents the middle products");
            dataTable.addCell("Z");
            dataTable.addCell("Items which require least controls, are sometimes issues as \"free stock\" or forward holding");
            ...
            secondMainHeaddingDatas.add(new Paragraph("    1) Identify the objective and the analysis criterion and determine the relevant items.\n"));
            secondMainHeaddingDatas.add(new Paragraph("    2) Collect all the data about the inventory and calculate the variation coefficients of each item.\n"));
            secondMainHeaddingDatas.add(new Paragraph("    3) Divide Item's current stock value (Cost per piece) with the total stock value (Total cost).\n"));
            secondMainHeaddingDatas.add(new Paragraph("    4) Arrange all the consumption values in descending order of values.\n"));
            secondMainHeaddingDatas.add(new Paragraph("    5) Create next column and start adding the cumulative total.\n"));
            secondMainHeaddingDatas.add(new Paragraph("    6) Assign the Category based on the accumulated value as X, Y & Z and plot a Graphical representation divided in to categories.\n"));
            ...
            bulletHeaddings12.add(new Phrase(" X – very little variance: X materials are characterized by a constant, non-changing usage over time. The requirements fluctuate only slightly around a constant"
                    + " level so that the future demand can basically be forecast quite well.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings22.add(new Phrase(" Y – some variance: The usage of Y materials is neither constant nor sporadic. With Y materials, we can often observe trends, for example, that the usage increases "
                    + "or decreases for a while, or that it is characterized by seasonal fluctuations. For these materials, it’s harder to obtain an accurate forecast.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings23.add(new Phrase(" Z – the most variation: Z materials are not used regularly. The usage can strongly fluctuate or occur sporadically. In these cases, we can often observe periods "
                    + "with no consumption at all.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            reportTable.addCell("Total");
            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("X")) + Integer.parseInt(request.getParameter("Y")) + Integer.parseInt(request.getParameter("Z"))));
            reportTable.addCell("");
            reportTable.addCell("");
            reportTable.addCell(request.getParameter("TOTAL_STOCK"));
            document.add(reportTable);
            document.add(new Paragraph(Chunk.NEWLINE));
            Chunk fourthMainHeadding = new Chunk("4.1 Sample graphical representation of " + analysisType + " Analysis:\n", new Font(customBaseFont, 14, Font.BOLD, BaseColor.BLACK));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingDatas = new Paragraph("After analyzing the data it is observed that the recommendation factors are like\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            ...
            fifthMainHeaddingData.add(new Paragraph("    X: Approx. " + request.getParameter("X_Total_Stock") + " (" + request.getParameter("X") + ") materials accounting for " + request.getParameter("X_COUNT") + "% i.e., " + request.getParameter("X_VALUE") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    Y: Approx. " + request.getParameter("Y_Total_Stock") + " (" + request.getParameter("Y") + ") materials accounting for " + request.getParameter("Y_COUNT") + "% i.e., " + request.getParameter("Y_VALUE") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    Z: Approx. " + request.getParameter("Z_Total_Stock") + " (" + request.getParameter("Z") + ") materials accounting for " + request.getParameter("Z_COUNT") + "% i.e., " + request.getParameter("Z_VALUE") + " Millions of the total consumption value\n"));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingDatas1 = new Paragraph("Organizations should never be out of stock on Class AX items. By using above comparison class AZ items likely would not be inventoried because "
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingData2 = new Paragraph("With the combination of ABC analysis to consider item value and XYZ analysis to factor in demand variation, inventory managers can then collaborate"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingData21 = new Paragraph("The departments should all work together to both establish inventory management policies and develop systems and processes to implement these policies. "
            ...
            bulletHeaddings34.add(new Phrase(" Mutually agreed upon inventory parameters for ABC items including buffer stocks.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            reportTable3.addCell("Automated with manual intervention\n Low buffer - accept stock out risk\n Perpetual inventory\n");
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddings43 = new Paragraph("XYZ analysis can be calculated by using the following procedure have to be done. \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddings48 = new Paragraph("Compilation of all these data can be identified and undergone in to troubles. \n", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddings52 = new Paragraph("The above formula may look simple for calculating SD. But if consider data of large organization with 3000-15000 SKU’s spread over a "
            ...
            bulletHeaddings54.add(new Phrase(" Quarterly\n", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings56.add(new Phrase(" Fortnightly\n", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings59.add(new Phrase(" Hourly etc \n", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings59);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph secondMainHeaddings60 = new Paragraph("Let’s starts to calculate Variance, SD and CV for monthly data i.e. data consolidated monthly and their monthly patterns as per the data "
                    + "study.\n", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            document.add(secondMainHeaddings60);
            document.add(new Paragraph(Chunk.NEWLINE));
            document.add(new Paragraph(Chunk.NEWLINE));
            document.add(new Paragraph(Chunk.NEWLINE));
            Chunk recommendedAnalysis = new Chunk("Recommended Other PiLog Products :", new Font(customBaseFont, 16, Font.BOLD, BaseColor.BLACK));
    ...
    public void fmsnPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            Image img = getImage(imageStr);
            img.scalePercent(70);
            img.setAlignment(com.itextpdf.text.Element.ALIGN_CENTER);
            ...
            dataTable.addCell("Items which are issued times in 5 years with exception of 1 or 2 years not used and / or intermittently used for remaining years and / or used 3-4 times in every 5 year\n");
            dataTable.addCell("S");
            dataTable.addCell("Slow Moving\n");
            dataTable.addCell("Items which are issued once or twice in every 5 years with extremely disjointed consumption\n");
            dataTable.addCell("N");
            dataTable.addCell("Non-Moving\n");
            dataTable.addCell("Items which are not issued in 5 years\n");

            Paragraph firstMainHeaddingData = new Paragraph();
            firstMainHeaddingData.add(new Paragraph("" + analysisType + "  Analysis is based on the rate of issue or rate of usage of spare parts. "
            ...
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("    1) Classification is based on the pattern of issues from stores and is useful in controlling obsolescence\n"));
            secondMainHeaddingData.add(new Paragraph("    2) Date of receipt or last date of issue, whichever is later, is taken to determine the no. of years which have lapsed since the last transaction\n"));
...
//            thirdMainHeaddingData.add(new Paragraph("    Based on the data provided for the analysis period, PiLog performed the analysis\n"));
            ...
            Paragraph fifthMainHeaddingData = new Paragraph();
            fifthMainHeaddingData.add(new Paragraph("    After analyzing the data, it is observed that\n"));
            fifthMainHeaddingData.add(new Paragraph("    F: Approx. " + request.getParameter("F_COUNT") + " (" + request.getParameter("F") + ") of the materials are active items which are ordered every year\n"));
            fifthMainHeaddingData.add(new Paragraph("    S: Approx. " + request.getParameter("S_COUNT") + " (" + request.getParameter("S") + ") of the materials are used once or twice in every 5 Years\n"));
            fifthMainHeaddingData.add(new Paragraph("    N: Approx. " + request.getParameter("N_COUNT") + " (" + request.getParameter("N") + ") of the materials whose consumption is nil or insignificant i.e.,    those are non-moving items\n"));
            ...
            reportTable.addCell("Total");
            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("F")) + Integer.parseInt(request.getParameter("S")) + Integer.parseInt(request.getParameter("N"))));
            reportTable.addCell("");
            reportTable.addCell("");
//            reportTable.addCell(request.getParameter("Total_CPP_Pct"));
            document.add(dataTable);
...
//            document.add(thirdMainHeaddingData);
    ...
    public void hmlPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            BaseColor backgroundBaseColor = new BaseColor(64, 135, 94);//green
//            BaseColor backgroundBaseColor=new BaseColor(0, 113, 197);//blue
            PdfPCell cell = new PdfPCell();
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("HML is another inventory analysis method. This method of analysis classifies inventory according to the materials unit price. HML method of inventory "
            ...
            secondMainHeaddingv1.add(new Paragraph("It is based on Pareto principle or the 80/20 rule. It is used to keep control over consumption at departmental level for deciding the frequency of "
                    + "physical verification.\n"
                    + "In this analysis cut-off-lines are then fixed by the management of the organization to classify the inventory items.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            dataTable.addCell("These items are low cost item as compared to H class items, this are generally 20-25% of total items");
            dataTable.addCell("L");
            ...
            Paragraph bulletHeaddings58 = new Paragraph();
            bulletHeaddings58.add(bullets);
            bulletHeaddings58.add(new Phrase(" Primary Objectives of HML is essential goal of this analysis is to limit the expense of Inventory.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings58);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings59 = new Paragraph();
            bulletHeaddings59.add(bullets);
            bulletHeaddings59.add(new Phrase(" The main objective of this analysis is to minimize the inventory cost such as labor cost, material cost etc. This analysis is similar to ABC analysis the difference "
            ...
            bulletHeaddings60.add(new Phrase(" Prepare the list of items and calculate their unit cost, annual demand and annual usage\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings62.add(new Phrase(" Calculate percentage of unit cost, cumulative of unit cost and then categories the inventory item. The cut off lines are then fixed by the organization for deciding"
                    + " three categories\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings63.add(new Phrase(" Plot the graph on the basis of cumulative of unit cost and then categories the inventory items", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            bulletHeaddings64.add(new Phrase(" The HML analysis is useful for keeping control over consumption at departmental levels", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings64);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings65 = new Paragraph();
            bulletHeaddings65.add(bullets);
            bulletHeaddings65.add(new Phrase(" HML method is deciding the frequency of physical verification and for controlling purchases.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings65);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings66 = new Paragraph();
            bulletHeaddings66.add(bullets);
            bulletHeaddings66.add(new Phrase(" HML analysis can determine the frequency of stock verification.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings66);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings67 = new Paragraph();
            bulletHeaddings67.add(bullets);
            bulletHeaddings67.add(new Phrase(" The objective of HML analysis is to keep control over the consumption at the department level. ", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletHeaddings67);
            document.add(new Paragraph(Chunk.NEWLINE));

            Paragraph bulletHeaddings68 = new Paragraph();
            bulletHeaddings68.add(bullets);
            bulletHeaddings68.add(new Phrase(" This method can evolve buying policy to control purchase of material in the organization level and delegate the authority to another buyer.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            secondMainHeadding22.add(new Paragraph("The high, medium and low (HML) analysis is same as ABC. Actually, the things are matters is that rather than the yearly utilization of more utilized in the ABC. "
            ...
            secondMainHeadding12.add(new Paragraph("    1) Prepare the list of items and calculate their unit cost, annual demand and annual usage\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            secondMainHeadding12.add(new Paragraph("    2) Arrange items in the decreasing order of their unit cost\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            secondMainHeadding12.add(new Paragraph("    3) Calculate percentage of unit cost, cumulative of unit cost and then categories the inventory item\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            secondMainHeadding12.add(new Paragraph("    4) The cut off lines are then fixed by the organization for deciding three categories\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            secondMainHeadding12.add(new Paragraph("    5) Plot the graph on the basis of cumulative of unit cost and then categories the inventory items\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("H")) + Integer.parseInt(request.getParameter("M")) + Integer.parseInt(request.getParameter("L"))));
            ...
            Paragraph fifthMainHeaddingData = new Paragraph();
            fifthMainHeaddingData.add(new Paragraph("    After analyzing the data, it is observed that\n"));
            fifthMainHeaddingData.add(new Paragraph("    H: Approx. " + request.getParameter("H_COUNT") + " (" + request.getParameter("H") + ") materials accounting for " + request.getParameter("H_CUM_PCT") + "% i.e., " + request.getParameter("H_VALUE") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    M: Approx. " + request.getParameter("M_COUNT") + " (" + request.getParameter("M") + ") materials accounting for " + request.getParameter("M_CUM_PCT") + "% i.e., " + request.getParameter("M_VALUE") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    L: Approx. " + request.getParameter("L_COUNT") + " (" + request.getParameter("L") + ") materials accounting for " + request.getParameter("L_CUM_PCT") + "% i.e., " + request.getParameter("L_VALUE") + " Millions of the total consumption value\n"));
            document.add(fifthMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            reportTablel.addCell("These items are low cost item as compared to H class items, this are generally 20-25% of total items");
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            document.add(new Paragraph(Chunk.NEWLINE));
            Chunk recommendedAnalysis = new Chunk("Recommended Other PiLog Products :", new Font(customBaseFont, 16, Font.BOLD, BaseColor.WHITE));
    ...
    public void sdePdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            BaseColor backgroundBaseColor = new BaseColor(64, 135, 94);//green
//            BaseColor backgroundBaseColor=new BaseColor(0, 113, 197);//blue
            PdfPCell cell = new PdfPCell();
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("The SDE analysis looks at inventory is available and classifies it according to the scarcity of supply. SDE Analysis - Based on Lead Time & Availability of Items.\n"
            ...
            fourthSubHeadingData.add(new Paragraph("    2) Collect all the data about the scarcity and availability of the items.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fifstMainHeaddingData = new Paragraph();
            fifstMainHeaddingData.add(new Paragraph("By using the SDE classification analysis the following importance can takes place\n"
                    ...
                    + "3.It helps in identify the items need to get in advance and which products can be sourced with ease. ", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(fifstMainHeaddingData);
            ...
            document.add(secondMainHeadding12);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingData = new Paragraph("One of the main benefits of conducting an SDE analysis is future planning. SDE analysis highlights the products might be requiring care and skill "
            ...
            document.add(secondMainHeadding22);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue1 = new Paragraph("SDE can make a confuse in inventory data won’t be clearer it isn’t always very accurate.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue1);
            Paragraph ninethMainHeaddingValue2 = new Paragraph("This analysis can help loosely with planning but one difficult product might vary drastically from another difficult product.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue2);
            Paragraph ninethMainHeaddingValue3 = new Paragraph("For instance, one could be acquired in three weeks while another one could take fourth months to procure. However, they are both marked D for difficult"
                    + ".\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue3);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeadding1 = new Paragraph("It can provide some overarching guidance but this classification shouldn’t be used to plan exact dates.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeadding1);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeadding = new Paragraph("Consulting with individual suppliers is necessary and each product needs to be managed on an individual basis.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeadding);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            reportTable.addCell("Total");
            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("S")) + Integer.parseInt(request.getParameter("D")) + Integer.parseInt(request.getParameter("E"))));
            reportTable.addCell("");
            reportTable.addCell("");
            document.add(reportTable);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            Paragraph fifthMainHeaddingData = new Paragraph();
            fifthMainHeaddingData.add(new Paragraph("    After analyzing the data, it is observed that\n"));
            fifthMainHeaddingData.add(new Paragraph("    S: Approx. " + request.getParameter("S_INPUT_PCT") + " (" + request.getParameter("S") + ") materials accounting for " + request.getParameter("S_COUNT") + "% i.e., " + request.getParameter("S_INPUT_PCT") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    D: Approx. " + request.getParameter("D_INPUT_PCT") + " (" + request.getParameter("D") + ") materials accounting for " + request.getParameter("D_COUNT") + "% i.e., " + request.getParameter("D_INPUT_PCT") + " Millions of the total consumption value\n"));
            fifthMainHeaddingData.add(new Paragraph("    E: Approx. " + request.getParameter("E_INPUT_PCT") + " (" + request.getParameter("E") + ") materials accounting for " + request.getParameter("E_COUNT") + "% i.e., " + request.getParameter("E_INPUT_PCT") + " Millions of the total consumption value\n"));
            document.add(fifthMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingnew4 = new Paragraph("SDE analysis classification represents three levels are Scarce, difficult and easy.\n"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue4 = new Paragraph("Items which are imported and those items which require more than 6 months lead time.\n"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue5 = new Paragraph("Items which require more than a within a day but less than 6 months lead time.\n"
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue6 = new Paragraph("Items which require more than a within a day but less than 6 months lead time.\n"
                    + "Lastly easily identifies inventory that is readily available and easy to access. These items are available locally "
                    + "and can be procured quickly.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            ...
            reportTable1.addCell("Items which are imported and require longer lead time ie., more than 6 months (180+ Days)");
            reportTable1.addCell("Longer Lead Time");
            reportTable1.addCell("D - Difficult");
            reportTable1.addCell("Items which require more than a fortnight to be available, but less than 6 months’ lead time (between 1 to 180 Days)");
    ...
    public void vedPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            BaseColor backgroundBaseColor = new BaseColor(64, 135, 94);//green
//            BaseColor backgroundBaseColor=new BaseColor(0, 113, 197);//blue
            PdfPCell cell = new PdfPCell();
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("VED analysis classifies the materials according to their criticality and impact to the production "
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValues = new Paragraph("VED Analysis can categorize the inventory materials under three heads which are VITAL, "
            ...
            document.add(thirdMainHeaddingnew6);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue4 = new Paragraph("VITAL category suggests that “Vital” includes the inventory which is necessary for production "
                    ...
                    + "place by the management to ensure the continuous availability of items under the “vital” category.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue4);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(thirdMainHeaddingnew7);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue5 = new Paragraph("IEssential category inventory items includes which is next to be place vital. These category "
                    ...
                    + "The unavailability of essential category inventory should not cause any stoppage or delays.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue5);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            document.add(thirdMainHeaddingnew8);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue6 = new Paragraph("Desirable category of inventory items is the least important among VED and the unavailability "
            ...
            dataTable1.addCell("The spares stock out of which even for a short time will stop production for quite some time & future the cost of stock out is very high");
            dataTable1.addCell("Average Working Hours: >= 4 Hours");
            dataTable1.addCell("E-Essential");
            dataTable1.addCell("The spares stock out of which even for a few hours of days & cost of lost production is high");
            dataTable1.addCell("Average Working Hours: >=1, < 4 Hours");
            dataTable1.addCell("D-Desirable");
            dataTable1.addCell("The spares which are needed but their absence for even for even a week or so will not lead to stoppage of production");
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingData = new Paragraph("VED stands for vital, essential and desirable. this analysis is used for classifying each "
                    + "stock (mainly spare parts) into above three category based on their requirement and stocked accordingly.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(thirdMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph thirdMainHeaddingDataS = new Paragraph("It helps in classifying items into three category and stocked accordingly. Vital items are "
            ...
            reportTable.addCell(request.getParameter("D_Count"));
            reportTable.addCell("Total");
            reportTable.addCell(String.valueOf(Integer.parseInt(request.getParameter("V")) + Integer.parseInt(request.getParameter("E")) + Integer.parseInt(request.getParameter("D"))));
            reportTable.addCell("");
            reportTable.addCell("");
            document.add(reportTable);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            Paragraph fifthMainHeaddingData = new Paragraph();
            fifthMainHeaddingData.add(new Paragraph("    After analyzing the data, it is observed that\n"));
            fifthMainHeaddingData.add(new Paragraph("    V: Approx. " + request.getParameter("V_COUNT") + " of the spares are very critical, failure of which leads to stoppage of production for more than 4 hours"));
            fifthMainHeaddingData.add(new Paragraph("    E: Approx. " + request.getParameter("E_COUNT") + " of the spares are critical for which lost production is high which leads to stoppage of production for 1-4 hours"));
            fifthMainHeaddingData.add(new Paragraph("    D: Approx. " + request.getParameter("D_COUNT") + " of the spares are non-critical which lead to short stop or production stop for less than an hour."));
            document.add(fifthMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            bulletMainHeadding1.add(new Phrase(" It is of utmost importance to any organization to maintain an optimum level of inventory.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding1);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeadding2 = new Paragraph();
            bulletMainHeadding2.add(bullet);
            bulletMainHeadding2.add(new Phrase(" VED analysis is a crucial tool to understand and categorize inventory according to its importance..\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding2);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeadding3 = new Paragraph();
            bulletMainHeadding3.add(bullet);
            bulletMainHeadding3.add(new Phrase(" Management can optimize costs by investing more in the vital and essential category materials..\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding3);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeadding4 = new Paragraph();
            bulletMainHeadding4.add(bullet);
            bulletMainHeadding4.add(new Phrase(" Maintaining inventory has its costs, and hence, this analysis bifurcates inventory in three parts to help "
            ...
            bulletMainHeadding6.add(new Phrase("Item Cost \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding6);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeadding7 = new Paragraph();
            bulletMainHeadding7.add(bullet);
            bulletMainHeadding7.add(new Phrase(" It is the actual purchase value of holding stock. This is the cost or price of the inventory items. "
            ...
            bulletMainHeadding9.add(new Phrase(" These may include transportation charges, packing charges, etc. "
            ...
            bulletMainHeadding10.add(new Phrase("Holding Costs \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding10);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeadding11 = new Paragraph();
            bulletMainHeadding11.add(bullet);
            bulletMainHeadding11.add(new Phrase(" After the purchase of inventory items, there are classified into few more costs. These may be related "
            ...
            bulletMainHeaddings.add(new Phrase("Vital: \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddings);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingv1 = new Paragraph();
            bulletMainHeaddingv1.add(bullet);
            bulletMainHeaddingv1.add(new Phrase(" Vital category includes the inventory items which is necessary for production or any other process in "
                    + "an organization.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeadding11);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingv2 = new Paragraph();
            bulletMainHeaddingv2.add(bullet);
            bulletMainHeaddingv2.add(new Phrase(" If the inventory of Vital item is unavailable in the market entire production may "
                    + "effects or stopped.\n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingv2);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingv3 = new Paragraph();
            bulletMainHeaddingv3.add(bullet);
            bulletMainHeaddingv3.add(new Phrase(" Vital items are very most important in the production process ordering such items should be "
            ...
            bulletMainHeaddingse1.add(new Phrase("Essential : \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingse1);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingse2 = new Paragraph();
            bulletMainHeaddingse2.add(bullet);
            bulletMainHeaddingse2.add(new Phrase(" Essential category of inventory items can be classified as the next to Vital and these materials "
            ...
            bulletMainHeaddingse3.add(new Phrase(" Essential materials are also keeping in stock always loss due to their unavailability may be "
            ...
            bulletMainHeaddingse4.add(new Phrase(" Ensure the essential material items are always in optimum availability and maintenance.\n",
                    new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingse4);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingsd = new Paragraph();
            bulletMainHeaddingsd.add(new Phrase("Desirable : \n", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingsd);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingsd1 = new Paragraph();
            bulletMainHeaddingsd1.add(bullet);
            bulletMainHeaddingsd1.add(new Phrase(" Desirable category of inventory materials is the least important among the VED.\n",
                    new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingsd1);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingsd2 = new Paragraph();
            bulletMainHeaddingsd2.add(bullet);
            bulletMainHeaddingsd2.add(new Phrase(" Unavailability of Desirable material may cause minor stoppages in production.\n",
                    new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(bulletMainHeaddingsd2);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph bulletMainHeaddingsd3 = new Paragraph();
            bulletMainHeaddingsd3.add(bullet);
            bulletMainHeaddingsd3.add(new Phrase(" The easiest way of maintaining these material shortages is possible in a short duration of time.\n",
                    new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
    ...
    public void acaPdfData(String imageStr, HttpServletRequest request, com.itextpdf.text.Document document, String analysisType, BaseFont customBaseFont) {
        try {
            BaseColor backgroundBaseColor = new BaseColor(64, 135, 94);//green
//            BaseColor backgroundBaseColor=new BaseColor(0, 113, 197);//blue
            PdfPCell cell = new PdfPCell();
...
//            Font f = new Font(customBaseFont, 16.0f, Font.BOLD, BaseColor.WHITE);
            ...
            document.add(new Paragraph(Chunk.NEWLINE));
            //onkar color
            Chunk firstSubHeaddingChunk = new Chunk("Equipment Criticality Analysis", new Font(customBaseFont, 14.0f, Font.BOLD, BaseColor.WHITE));
            firstSubHeaddingChunk.setBackground(backgroundBaseColor, 0, 3, 343, 3);
            //color
            Paragraph firstSubHeadding = new Paragraph(firstSubHeaddingChunk);
//            Paragraph firstSubHeadding = new Paragraph("Material Criticality Analysis\n", new Font(customBaseFont, 14, Font.BOLD, BaseColor.BLACK));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstMainHeaddingData = new Paragraph();
            firstMainHeaddingData.add(new Paragraph("   Equipment critical analysis is a quantitative analysis of equipment faults, and ranking them in order of serious consequences on safety, environment, production loss and maintenance cost. "
                    ...
                    + "Purpose of Equipment Criticality Analysis", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(firstMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph firstSmallSubHeadingData = new Paragraph("The following are the various methods to control the inventory process:", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
//            firstSmallSubHeadingData.setAlignment(Element.ALIGN_LEFT);
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph secondMainHeaddingData = new Paragraph();
            secondMainHeaddingData.add(new Paragraph("    Which equipment has the most serious potential consequences on business performance, 'if it fails'? The resulting Equipment Criticality Number is used to prioritize resources performing maintenance work.\n"
...
//            
            String equiCriticalityRankingImage = request.getServletContext().getRealPath("/images/Equipment_Criticality_Ranking.png");
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            ...
            String assCriticalityScoring = request.getServletContext().getRealPath("/images/ASSET_CRITICALITY_SCORING.png");
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fourthSubHeadingData = new Paragraph();
            fourthSubHeadingData.add(new Paragraph(" Weighted criticality criteria scores are multiplied to determine the asset\n"
            ...
            fourthSubHeadingData.add(new Paragraph("             Criticality Index\n"
                    + "                       = Assert Failure × Asset Importance\n"
                    + "                                       × Asset Reliability × Asset Utilization ", new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.ITALIC, BaseColor.RED)));
            fourthSubHeadingData.add(new Paragraph(" ACA scoring criteria ranges from 0, indicating high criticality against the criterion, to 4, indicating low criticality against the criterion\n"
                    + "", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph fifstMainHeaddingData = new Paragraph();
            fifstMainHeaddingData.add(new Paragraph(" SAP- Plant Maintenance (SAP-PM) Module, where Class A represents ‘Critical’ asset, Class B represents ‘Important’ asset, and Class C represents ‘Ordinary’ Asset", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK)));
            document.add(fifstMainHeaddingData);
            document.add(new Paragraph(Chunk.NEWLINE));
            PdfPTable dataTable = new PdfPTable(3);
            dataTable.setTotalWidth(100f);
            ...
            //onkar added
            Paragraph cellvalue = new Paragraph();
            cellvalue = new Paragraph("SAP-PM Criticality Class", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph(" Criticality Index (CI) Range", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph(" Equipment\n" + "Criticality", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Class A", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph((String) request.getParameter("C_RANGE"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Critical Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Class B", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph((String) request.getParameter("I_RANGE"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Important Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Class C", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph((String) request.getParameter("O_RANGE"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            cellvalue = new Paragraph("Ordinary Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable.addCell(cellvalue);
            ...
            cellData = new Paragraph("Equipment Criticality", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            dataTable1.addCell(cellData);
            cellData = new Paragraph(" Equipment Count ", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));
            dataTable1.addCell(cellData);
            cellData = new Paragraph(" Equipment Percentage(%)", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph("Critical Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("C_Count"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("C_Percentage"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph("Important Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("I_Count"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("I_Percentage"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph("Ordinary Equipment", new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("O_Count"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            cellData = new Paragraph((String) request.getParameter("O_Percentage"), new Font(customBaseFont, 12, Font.NORMAL, BaseColor.BLACK));//onkar added
            dataTable1.addCell(cellData);
            ...
            document.add(ninethMainHeadding);
//            document.add(new Paragraph(Chunk.NEWLINE));
            Paragraph ninethMainHeaddingValue = new Paragraph("A score of zero ‘0’ in any of the ACA criteria, shall immediately rank the asset as a ‘Critical’ asset. Therefore, any HSE or SCE shall automatically be ranked as Critical Asset thru the ACA methodology. Assets identified by risk assessment studies as HSE critical or AIMS SCE shall has major impact on HSE and consequently, by default, will receive a score of 0 for its Failure Consequences in ACA Matrix.", new Font(customBaseFont, 14, Font.NORMAL, BaseColor.BLACK));
            document.add(ninethMainHeaddingValue);
            document.add(new Paragraph(Chunk.NEWLINE));
...
//            document.add(new Paragraph(Chunk.NEWLINE));
    ...
    public String getDataVisualizationLayout(HttpServletRequest request) {
        String result = "";
        String Connection = "Test";
        try {
            String treeId = request.getParameter("treeId");
            ...
            result = "<div class='dxpDataAnalyticswrapper'>"
                    + "<div class='leftFileUploads width60' id=\"leftFileUploadMainDivwrapperID\">"
                    + "<div class=\"dqopsExstingTablesWrapper\">"
                    + "<div class=\"dqopsExstingTablesItem Panel_A\">"
                    + "<div class=\"leftUploadHeaderDiv\" onclick=\"dataUploadsDivToggle()\">"
                    + "<span class=\"toggleImg\" id=\"columnsToggleIcon\" onclick=\"dataIntegrationGuide()\"><img src=\"images/toggle_plusicon.png\" width=\"16px;\">"
                    + "</span>"
                    + "</div>"
                    + "<div id='dBConnection' class='dBConnectionClass'>" + "<ul id='dxpConnection'>"
                    + "<a title='New Connections' id='visualConnectionLi' >"
                    + "<img src='images/New Connection Icon-01.svg' onclick=showVisualizationConnection() class='visionEtlTabIcons visualDarkMode' style='cursor:pointer;'>"
                    + "</a>" + "</ul>" + "</div>"
                    + "<div class='visionVisualizationDataSourcesCLass fileUploadsDA' id='visionVisualizationDataSourcesId'>"
                    + "<div id=\"VisualizationSources\" class ='VisualizationSourcesCLass'></div> "
                    ...
                    // class='ivisualizationConnectionsClass' style='display:none' >"
                    // +"</div>"
                    + "<div id=\"dqopsExstingConnections\" class=\"dqopsExstingTablesItem Panel_B\">"
                    + "<div class=\"leftUploadHeaderDiv\" onclick=\"dataUploadsDivToggle()\">"
                    + "<span class=\"toggleImg\" id=\"columnsToggleIcon\" data-flag=\"A\" onclick=\\\"dataIntegrationGuide()\\\"><img src=\"images/toggle_plusicon.png\" width=\"16px;\">"
                    + "</span>"
                    + "</div>"
                    + "<div class='dBConnectionSecClass'>" + "<ul id='dxpConnection'>"
                    + "<a title='Available Connections' id='treeDxpConnectionLi' >"
                    + "<img src='images/tree.svg' onclick=treeDxpConnections() class='visionEtlTabIcons visualDarkMode' style='cursor:pointer;'>"
                    + "</a>"
                    + "</ul></div>"
                    + "<div id=\"ivisualizationConnectionsMain\" class='ivisualizationConnectionsMainClass' >"
                    + "<div id=\"savedConnIcons\" class=\"savedConnIconsClass\">"
                    + "<img src=\"images/Refresh Icon.svg\" class=\"visionETLIcons visualDarkMode\" title=\"Refresh\" style=\"width:15px;height: 15px;cursor:pointer;\" "
                    + "onclick='refreshMappingTablesAnalytics()'/>"
                    + "<img src=\"images/Filter Icon-01.svg\" class=\"visionETLIcons visualDarkMode\" id=\"treeETLFilterImage\" title=\"Filter\" style=\"width:15px;height: 15px;cursor:pointer;\""
                    + "onclick='filterMappingTablesAnalytics()'/>" + "</div>"
                    + "<div id=\"connectionSearchId\" class=\"connectionSearchId\"><input type=\"search\" id=\"valueId\" class=\"connectionSearchInputCls\" onkeypress=\"showDataBaseTablesData(event)\" name=\"search\" placeholder=\"Search here...\"><button onclick=\"getFilteredSchemaObject()\" id=\"searchSchemaObjBtnId\" class=\"searchSchemaObjBtnCls\"><i class=\"fa fa-search\"></i> </button></div>"
                    + "<div id=\"ivisualizationConnections\" class='ivisualizationConnectionsClass'>" + "</div>"
                    + "</div>"
                    + "</div>"
                    + "<div id=\"visualizationMainDivwrapperID\" class='visualizationMainDivwrapper width60'>"
